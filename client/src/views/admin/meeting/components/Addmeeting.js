...
const AddMeeting = (props) => {
    ...
    const AddData = async (values) => {
        ...
        try {
            const response = await postApi('api/meeting', formik.values);
    ...
    useEffect(() => {
        if (props.id && props.leadContect) {
            props.leadContect === 'contactView'
                ...
                : props.leadContect === 'leadView' && setLeadData([props.leadDetails]);
        } else {
            fetchAllData();
        }
    }, [values.related, props.id, contactList, leadData, props.leadContect]);
